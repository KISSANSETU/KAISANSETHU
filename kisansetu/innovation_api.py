"""KisanSetu SIH innovation layer.
...
router = APIRouter(prefix="/api/v2", tags=["KisanSetu SIH 2026"])
...
PAYMENT_DEMO_MODE = os.environ.get("KISANSETU_PAYMENT_DEMO", "1") == "1"
...
def init_innovation_schema():
    ...
    # ============================================================
    # Existing KisanSetu databases already contain the basic
...
class FeedbackIn(BaseModel):
    ...
    message: str = Field(
        default="Farmer feedback submitted through KisanSetu",
        max_length=4000
    )
    ...
    try:
        ...
        cur = c.execute(
            ...
            (
                ...
                x.message.strip()
                if x.message.strip()
                else "Farmer feedback submitted through KisanSetu",
...
def create_excess_refund(c,payment_id,amount_paise,gateway_payment_id):
    ...
    if gateway_payment_id.startswith("pay_demo_") or PAYMENT_DEMO_MODE:
        ...
        try:
            out=razorpay_request("POST",f"/payments/{gateway_payment_id}/refund",{"amount":amount_paise,"notes":{"reason":"KisanSetu automatic overpayment refund"}})
...
def chat(q:str,state:str=DEFAULT_STATE,lang:str="en",u=Depends(get_user_dep())):
    ...
    T={
      "en":{
        "market":f"For {state}, I can help you decide SELL NOW, WAIT, SHIFT MARKET or STORE. Open Crops & Forecasts, choose crop and market, then run Forecast + Best Market. KisanSetu compares 1/3/7-day prices with transport and market charges.",
        "preorder":"In Pre-Orders, KisanSetu compares the buyer offer with the 1/3/7-day outlook, transport, charges and buyer reliability, then suggests ACCEPT, NEGOTIATE or WAIT.",
      ...
      "hi":{
        ...
        "preorder":"Pre-Orders में KisanSetu खरीदार के ऑफर की 1/3/7 दिन के अनुमान, परिवहन लागत, बाजार शुल्क और खरीदार विश्वसनीयता से तुलना करके ACCEPT, NEGOTIATE या WAIT सुझाता है।",
      ...
      "mr":{
        ...
        "preorder":"Pre-Orders मध्ये KisanSetu खरेदीदाराची ऑफर 1/3/7 दिवसांचा अंदाज, वाहतूक, बाजार शुल्क आणि खरेदीदार विश्वसनीयतेशी तुलना करून ACCEPT, NEGOTIATE किंवा WAIT सुचवतो.",
      ...
      "ta":{
        ...
        "preorder":"Pre-Orders பகுதியில் வாங்குபவர் சலுகை, 1/3/7 நாள் கணிப்பு, போக்குவரத்து செலவு, சந்தை கட்டணம் மற்றும் வாங்குபவர் நம்பகத்தன்மை ஆகியவற்றை KisanSetu ஒப்பிட்டு ACCEPT, NEGOTIATE அல்லது WAIT பரிந்துரைக்கும்.",
...
# ============================================================
# KisanSetu V3 - role-complete prototype extensions
# ============================================================
...
def v3_order_tracking(order_id:int,u=Depends(get_user_dep())):
    ...
    if not rows:
      stages=[('ORDER_PLACED','Order created in KisanSetu','Seller location'),('TOKEN_CONFIRMED','Payment/token verification checkpoint','Secure payment'),('PICKUP_SCHEDULED','Transport pickup scheduled','Farmer pickup point'),('IN_TRANSIT','Track once transporter starts route','GPS logistics'),('DELIVERED','Final delivery confirmation','Buyer destination')]
...
def v3_admin_message(user_id:int,x:AdminMessageIn,u=Depends(get_user_dep())):
    require_role(u,'admin');c=conn();c.execute("INSERT INTO notifications(user_id,title,message,severity,created_at) VALUES(?,?,?,?,datetime('now'))",(user_id,'Message from KisanSetu Admin',x.message,x.severity));c.commit();c.close();return {'status':'SENT'}
