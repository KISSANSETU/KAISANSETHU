...
KNOWLEDGE = {
    ...
    "net_realizable_price": (
        ...
        "KisanSetu ranks markets by this take-home value, not by headline mandi price."),
    "sell_wait_shift": (
    ...
    "kyc": (
        "KYC verification is available after login. KisanSetu never stores a full Aadhaar "
...
def t_add_produce(u, args):
    ...
    if not canon:
        return {"error": f"'{crop}' is not a crop KisanSetu tracks.",
...
TOOLS = {
    ...
    "list_crops_and_markets": (t_list_crops_and_markets, {
        "description": "List every crop KisanSetu tracks and every mandi/market in a state. "
    ...
    "explain_platform": (t_explain_platform, {
        "description": "Explain how a KisanSetu feature works: predictability_score, "
...
def system_prompt(u, lang, state):
    ...
    return (
        "You are GRAM Saathi, the assistant inside KisanSetu, an Indian "
        ...
        "the local numeral script. Keep the product names KisanSetu, GRAM Saathi, "
