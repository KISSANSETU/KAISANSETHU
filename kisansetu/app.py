...
SECRET = os.environ.get("KISANSETU_SECRET","CHANGE-ME-IN-PRODUCTION-KISANSETU-2026")
ALG = "HS256"
TOKEN_MINUTES = 180

SMTP_HOST=os.environ.get("KISANSETU_SMTP_HOST","")
SMTP_PORT=int(os.environ.get("KISANSETU_SMTP_PORT","587"))
SMTP_USER=os.environ.get("KISANSETU_SMTP_USER","")
SMTP_PASSWORD=os.environ.get("KISANSETU_SMTP_PASSWORD","")
SMTP_FROM=os.environ.get("KISANSETU_SMTP_FROM",SMTP_USER or "no-reply@kisansetu.local")

app=FastAPI(title="KisanSetu Whole Project API",version="1.0.0")
try:
...
def request_phone_otp(
    ...
    if not account:
        raise HTTPException(
            status_code=404,
            detail=(
                "No KisanSetu account is linked "
...
async def inspect_produce(
    ...
    certificate_number = (
        f"KISANSETU-QC-{verification_id:06d}"
    )
...
def get_quality_certificate(
    ...
    try:
        path = generate_quality_certificate(
            certificate_number=row["certificate_number"] or f"KISANSETU-QC-{verification_id:06d}",
